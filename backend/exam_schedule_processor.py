...
def normalize_date(input_str: str) -> str:
    """Attempt 1, try to parse as existing Excel numeric date"""
    try:
...
def looks_like_excel_date(value: float) -> bool:
    """Excel dates are large integers"""
    return 1.0 <= value <= 100000.0 and value == int(value)


def format_excel_time(excel_time: float) -> str:
    """Excel stores time as fraction of a day, not useful to the user so it needs conversion
    0.5 = 12:00 PM, 0.25 = 6:00 AM, and so on"""
...
def filter_columns(data: List[List[str]]) -> List[List[str]]:
    ...
    #Filter data starting from header row (skipping the nonsense rows)
    filtered_data = []
